...
def log_metrics(birthdays_count, message_count, error_count=0):
    ...
    try:
        ...
        # Import boto3 only when needed to avoid module-level import issues in tests
        try:
            import boto3
            
            # Use environment variable for region with fallback to eu-west-2
            ...
            # Don't fail if CloudWatch isn't available (like in tests)
            logger.warning(f"Could not send metrics to CloudWatch: {str(e)}")
    except ImportError:
        # Handle case where boto3 might not be available
        logger.warning("boto3 not available, skipping CloudWatch metrics")

def get_todays_birthdays():
    """
    Get birthdays for today's date
    """
